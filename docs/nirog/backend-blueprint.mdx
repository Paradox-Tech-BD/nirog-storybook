...
The FastAPI application should use domain modules rather than feature files that directly access each other’s tables. Each module exposes services or repository interfaces. This gives a clean path to extracting a service later, while keeping the MVP simple.

| Module | Owns | Main responsibilities |
|---|---|---|
| `identity` | Accounts, tokens, devices, sessions. | Login, refresh token rotation, device registration, account lifecycle. |
| `profiles` | Patient profiles, profile access, consent records. | Household/dependent profiles, caregiver sharing, permissions, revocation. |
| `documents` | Prescription documents, pages, assets. | Upload validation, storage capability issuance, retention and deletion state. |
| `scan_orchestration` | Scan jobs, stage runs, progress events. | Queueing, retries, idempotency, result materialization, error state. |
| `ml_evidence` | OCR lines, structured fields, candidate matches, evidence references. | Persist and serve machine-generated evidence for user review. |
| `medicine_catalog` | Products, manufacturers, ingredients, aliases, catalog releases, index releases. | Search, product resolution, source provenance, active/inactive facts. |
| `regimens` | Regimen items, versions, schedules, planned doses. | Create/modify a confirmed medication course and generate a device schedule specification. |
| `adherence` | Dose events, notification events, adherence projections. | Record taken/skipped/snoozed actions and calculate app-recorded adherence. |
| `inventory` | Supply movements, refill rules, refill alerts. | Advisory remaining-supply estimate and refill prompt workflow. |
| `sync` | Device commands and profile-scoped change feed. | Exactly-once command ingestion, conflict response, ordered delta synchronization. |
| `curation` | Curation cases, source approval, alias review. | Controlled publication of shared catalog updates and release rollback. |
| `governance` | Audit, provenance, model/prompt/policy releases, evaluation runs. | Explainability, release approval, data lifecycle, operations metrics. |
