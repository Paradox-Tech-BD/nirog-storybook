...
**Status:** Phase 8 evidence foundation and the authenticated worker lease/result/review boundary are implemented, verified, and deployed; Core commit `863aa1c` adds explicit OCR result provenance.

**Scope:** profile-bound prescription evidence, constrained Cloudflare R2 upload authorization, durable OCR job creation, a narrow internal worker boundary, and machine-readable demo/ML provenance.
...
| Safe lifecycle metadata and bounded candidate extraction pending review | OCR text, confidence, prompts, or model-provider output in audit/outbox payloads |

## 2. Clinical records and lifecycle

The deployed foundation adds `clinical.prescription_evidence`, `clinical.ocr_jobs`, and `clinical.ocr_extractions`, with RLS predicates identical in principle to the deployed medication records. Evidence belongs to exactly one profile and one prescription. OCR jobs are profile-bound denormalized records for policy clarity and query efficiency; the foreign-key chain remains mandatory. Extraction rows are created only through the authenticated worker result boundary and remain pending review until an authorized human decision.
...
| `ocr_extractions` | `pending_review`, `accepted`, `rejected`, `superseded` | Raw extracted text, bounded candidate fields, `result_source`, and optional `demo_fixture_id`, retained only in the clinical schema | Results are advisory; provenance is mandatory (`ml` with no fixture or `demo` with a `demo.*` fixture), never appears with patient text in audit/outbox payloads, and never mutates a regimen automatically. |
...
| OCR-extraction listing | `document.read_metadata` and `regimen.read` | Returns bounded candidate fields, review status, and explicit result provenance; never performs an automatic regimen mutation. |
...
The worker is not permitted to query clinical tables directly, choose another evidence object, accept a patient profile from a client, or write a regimen. It receives an independently authenticated Core job lease and only a short-lived read authorization for the evidence object selected by that job. The result command accepts bounded candidate fields, a controlled status, and a provenance marker; `resultSource: "demo"` requires a fixture identifier matching `demo.[A-Za-z0-9._-]{1,127}`, while `resultSource: "ml"` forbids a fixture identifier. Any raw extraction is stored only in the clinical extraction record. Demo results are test artifacts and cannot become medication authority or trigger clinical actions. Production Core fails closed when R2 evidence processing is enabled without a sealed `NIROG_INTERNAL_WORKER_SECRET`.

## 6. Audit, outbox, and review invariants

Every evidence authorization, completion, OCR enqueue, lease, result, retry, rejection, and review decision emits audit/outbox evidence in the same transaction where applicable. These records may contain safe identifiers, state codes, byte-size category, controlled failure code, and the non-sensitive `resultSource`/`demoFixtureId` provenance markers only. They must not contain a raw evidence key, URL, filename, media bytes, OCR text, prompt, model name, provider request ID, checksum, device token, or generated medication instruction.

OCR results are advisory evidence. A later reconciliation command, authorized through `regimen.write`, may allow a user to create or update a manual regimen after explicit review. Demo results remain visibly marked throughout storage, responses, audit metadata, and identifier-only outbox events. This slice creates no automatic medication instruction and does not send reminders or notifications.
