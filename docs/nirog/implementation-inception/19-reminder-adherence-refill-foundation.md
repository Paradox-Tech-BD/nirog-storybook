...
**Status:** schema foundation, local-time materialization, due-intent dispatch, inventory/refill API, migration 0014, persisted adherence projections, authorized adherence reporting, reminder state transitions, dose-linked inventory deduction, and refill-alert acknowledgement are implemented and deployed in the Railway test environment. Controlled authenticated smoke tests, database-backed concurrency/RLS checks, weekly/monthly adherence read models, and refill-history reads remain open.
...
The existing Railway dispatcher is the selected execution host for the Phase 9 deterministic worker. This avoids an external polling product and reuses the already deployed PostgreSQL retry, lease, observability, and identifier-only outbox conventions. The deployed materializer has the dedicated `reminder-dispatcher` workload and the narrow `reminder.materialize` purpose; the due-dispatch slice now claims due rows atomically, emits only identifier-only `reminder.due.v1` intents, and marks delivery in the same database transaction under the `reminder.due.claim` and `reminder.due.complete` boundaries. Railway has `REMINDER_DUE_WORKER_ENABLED=true` for the dispatcher test workload.
...
Inventory is intentionally separate from adherence while still allowing a deterministic source-linked projection. Recording a `taken` or `late` dose outcome now creates one idempotent `dose_consumption` movement using the regimen’s stored dose quantity and the dose-log identifier as the uniqueness source. `missed` and `skipped` outcomes do not consume stock, and no movement is created when inventory tracking is inactive. The inventory API supports profile-authorized read, idempotent initialization, positive refill movements, and refill-alert acknowledgement.

`clinical.regimen_inventories` is the current balance projection and optional refill threshold for one regimen. `clinical.inventory_movements` is the append-only accounting record: `refill`, `adjustment`, `dose_consumption`, `reversal`, quantity delta, and before/after balances. The deployed refill command uses decimal-safe arithmetic, rejects a transition that would make the balance negative, and opens a refill alert only when a balance crosses downward through its configured threshold. A refill history is therefore a filtered ledger view rather than a mutable separate log.
...
The deployed inventory increment adds the profile-authorized `/inventory`, `/inventory/refills`, and `/inventory/refill-alerts/:alertId/acknowledge` API surface. Mutations require an idempotency key, use decimal quantities, write audit records with identifiers only, and publish no balance or medicine-name text in event payloads. Taken and late dose recording now deducts stock in the same medication transaction through a source-dose-linked ledger movement; a downward threshold crossing emits an identifier-only refill-alert event, and an authorized inventory mutation can acknowledge the alert without changing balance.

The medication-domain package now contains pure, tested daily-adherence and streak calculations. Migration `0014_adherence_timezone_metric_key.sql` makes the daily metric identity `(regimen_id, local_date, timezone)` so one regimen can be projected independently in different IANA timezone contexts. Dose recording now recomputes and persists the local-day metric and streak in the same medication transaction. Authorized reporting exposes the persisted daily metrics and streak projection, while weekly and monthly summaries are exposed as bounded aggregate read models rather than separate mutable tables.

The due-row/outbox flow, authorized snooze and acknowledgement transitions, idempotent dose-linked inventory deduction, refill-alert acknowledgement, weekly/monthly adherence summaries, and the profile-scoped refill-history read model are implemented and covered by Core tests. Core commit `a614eed` adds the authorized `GET /api/v1/profiles/:profileId/regimens/:regimenId/inventory/movements` route with a newest-first, bounded ledger projection; its Railway API deployment was approved after the migrator remained completed, and the matching OCR worker deployment was also approved in the test environment. The final operational acceptance checks are complete in the test environment: the authenticated synthetic OCR smoke path returned HTTP 200, the two-connection due-row race produced exactly one winner and one identifier-only outbox event, the concurrent refill-alert race left exactly one open/acknowledged alert, and the database-backed profile-RLS suite passed all 9 tests. The test database required a source-aligned ACL/RLS reconciliation because migrations had been recorded before the `nirog_api` role existed; this was a test-environment state repair only.

The executable Phase 9 slice now proves local-time materialization across ordinary and daylight-saving dates, duplicate-materialization prevention, authorized snooze and acknowledgement transitions, deterministic due-dispatch count behavior, source-linked inventory deduction behavior, refill-alert acknowledgement, weekly/monthly aggregation, newest-first refill-history projection, profile-scoped API responses, and idempotency-key enforcement. The final database-backed and authenticated operational checks now also pass: concurrent due-row claim safety, one-open-refill-alert behavior, profile-RLS isolation, and the bounded authenticated synthetic OCR smoke path.
