...
|---|---|---|
| `catalog.releases`, `catalog.sources`, `catalog.index_releases` | Medicine Catalog | Makes product facts and OCR match behavior reproducible. |
| `catalog.products`, `catalog.manufacturers`, `catalog.dosage_forms` | Medicine Catalog | Represents a product/presentation available in the reference catalog. |
| `catalog.ingredients`, `catalog.product_ingredients`, `catalog.drug_classes`, `catalog.indications` | Medicine Catalog | Represents pharmacological reference relationships rather than a patient’s instructions. |
...
| `regimen.schedule_policies` | Medication Regimen | Canonical schedule meaning; it publishes schedule changes for projection. |
| `adherence.planned_dose_occurrences` | Adherence and Notifications | Derived future administrations linked to the source regimen version; rebuildable without rewriting historical dose evidence. |
