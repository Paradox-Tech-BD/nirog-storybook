...
This is the merged application model based on the original Nirog class diagram. It retains the important user-facing concepts—`User`, `UserPreferences`, `PatientProfile`, `OCRScan`, `Reminder`, `DoseLog`, `RefillAlert`, `RefillHistory`, `Notification`, and `AdherenceStats`—while using the backend model needed for a real prescription-intelligence workflow.
...
classDiagram
    ...
    PatientProfile "1" --> "0..*" MedicationReview : confirms
...
| `UserProfile` | `PatientProfile` + `ProfileAccess` | Renamed to clarify that this is the personal/dependent medication context; profile sharing is added explicitly. |
