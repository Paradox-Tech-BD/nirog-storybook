...
- [x] Preserve LocalStack/SQS only for local asynchronous event emulation; do not conflate it with production evidence storage.
...
The backend now separates `EVENT_*` SQS transport configuration from `EVIDENCE_R2_*` evidence-object configuration. A new `@nirog/evidence-storage` package provides the Cloudflare R2 S3-compatible adapter with normalized-key, content-type, and presigned-URL expiry guards. Production requires `EVIDENCE_STORAGE_DRIVER=r2`, an HTTPS R2 endpoint, `auto` region, a bucket, and bucket-scoped credentials; local Compose defaults evidence storage to disabled and LocalStack to SQS only. `pnpm verify` passed with 16 tests across 6 files, OpenAPI regenerated successfully, and the Node 22 Storybook build passed. Live R2 credential, bucket CORS, and real-upload verification remain an environment integration step requiring user-supplied R2 values.

## Railway and AWS SQS deployment configuration

- [x] Add explicit Railway/AWS SQS identifier and IAM-credential variables to the deployment template and production validation.
- [x] Document where to create the queues, queue URLs, IAM access key, least-privilege policy, and Railway secret variables without exposing credentials in Git.
- [x] Verify the configuration guard and publish the Railway-ready environment guide.

The deployment contract now distinguishes SQS resource identifiers from AWS credentials. Railway production uses `EVENT_AWS_CREDENTIAL_SOURCE=environment` and sealed standard `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`; the config loader refuses that source when the credential pair is incomplete. The API/dispatcher publisher requires only `sqs:SendMessage` on `nirog-events`; a later worker must use a separate receive/delete/visibility identity. Queue URLs are non-secret service variables copied from AWS SQS, while R2 credentials remain separate sealed Cloudflare values. Backend `pnpm verify` passed with 18 tests across 6 files, the OpenAPI snapshot regenerated successfully, and the Node 22 Storybook build passed.
