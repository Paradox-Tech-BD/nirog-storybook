...
- [x] Repair the Web access guard so `owner` access retains normal owner actions even when the self-only access-context permission array is intentionally sparse; retain strict `document.create` and `regimen.write` checks for delegated grants.
...
- [x] Verify persisted editable-draft counts and confidence-state aggregates, plus authorized caregiver visibility, then publish only the outcome categories and implementation details needed for future maintenance.
- [x] Run one live check through a user-authorized temporary delegated-caregiver session: confirm self-only profile context and aggregate evidence/draft visibility, confirm absence of `regimen.write`, and do not create, edit, submit, or elevate any care relationship or clinical record.
- [x] Create only if needed a temporary least-privilege caregiver test identity and time-bounded delegated grant; verify live aggregate visibility and denied regimen write, then revoke or expire the grant without modifying clinical data or a medication regimen.
- [x] Repair the caregiver-facing evidence workspace so a delegated session without `regimen.write` receives aggregate provenance and draft visibility only: do not render editable medication fields or regimen-submit controls, and do not surface a manager-only access-roster read failure as a caregiver workflow error.
- [x] Recover the owner browser authentication flow without clearing Google or Clerk sessions, then revoke the temporary delegated caregiver grant and remove the synthetic test identity if the supported dashboard control is available.
- [x] Add the supported profile-grant DELETE path and DELETE method to the same-origin Core proxy, then use it only to revoke the temporary verification grant and verify the cleanup response.
- [x] Preserve bodyless upstream success responses such as `204` in the Core proxy so a successful temporary-grant revocation is not converted into a gateway error by response rewrapping.
- [x] Delete only the user-confirmed synthetic Clerk development test identity after its temporary delegated grant is revoked; do not modify any other Clerk user.
...
> **Live delegated-caregiver verification — 23 August 2026:** a dedicated Clerk development test identity was created solely for this verification, automatically projected through the ordinary Web/Core account bridge, and granted one owner-created, 30-minute `caregiver` relationship. Its self-only Core access context returned `grant` / `caregiver` with eight delegated permissions and no `regimen.write` or `share.manage`. The recipient could see only aggregate evidence, extraction, confidence, and draft state. The live check found a presentation defect: the browser still rendered editable draft controls and regimen-submit buttons even though Core would deny the mutation. Web commit `76bf37a` corrected this by rendering seven read-only draft summaries with zero editable clinical controls, zero regimen-submit controls, no file chooser, and no manager-roster warning. Temporary-grant cleanup exposed two proxy defects—an absent DELETE route and incorrect bodyless-204 response rewrapping—fixed and deployed in `aa0bf4f` and `0abe2ba`. The owner then revoked the grant through the same-origin proxy with a `204` result, and the user-confirmed synthetic Clerk identity was deleted. No prescription value, image, clinical field, draft edit, regimen creation, or regimen submission occurred during this verification.

> **Owner upload-path clarification — 23 August 2026:** the read-only screenshot was the intentionally limited delegated-caregiver presentation, not the owner workflow. A follow-up production check found that the owner self-context correctly reported `owner` but intentionally carried a sparse permission array; the Web guard had incorrectly treated that sparse snapshot as read-only. Web commit `ee23808` now treats Core `owner` access as sufficient for normal owner upload and explicit-regimen presentation, while a delegated grant still requires the exact `document.create` or `regimen.write` permission. The deployed owner workspace verified one profile, `owner` access, one native file selector, one enabled automatic-extraction action, an enabled new-prescription action, and zero read-only badges. No file was attached, no upload was started, and no regimen action was used in this check.

> **Latest bounded-rejection diagnosis and repair — 23 August 2026:** the newly supplied owner-authorized JPEG was selected and submitted exactly once through the ordinary production upload workflow. Aggregate metadata showed a supported `image/jpeg` record of 178,870 bytes; Core later persisted the terminal `rejected` state with zero extraction and zero medication drafts. Redacted runtime diagnostics found a retryable Gemini provider `503` followed by a Core result-callback `400`, rather than a browser upload, R2 authorization, dispatcher-delivery, or permanent provider-input rejection. Source inspection proved the worker serialized unread optional candidate fields as JSON `null`, while the Core TypeBox contract permits those fields only when omitted. Core commit `e50b4d3` removes null-valued optional fields before the private result callback and adds a focused regression test; all 26 isolated ML-worker tests passed and the targeted Railway worker deployment reached success. The terminal record was not manually requeued, no second upload was attempted, no medication content was read or edited, and no regimen was created or submitted. This repair is validated at the HTTP payload boundary; a new end-to-end success is intentionally not claimed until a future user-authorized automatic run completes against an available provider.
